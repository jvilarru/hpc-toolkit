...
locals {
  ...
  munge_disk = [{
    source_snapshot = var.munge_disk != null ? data.google_compute_snapshot.munge_snapshot[0].id : null
    boot            = "false"
    auto_delete     = true
    device_name     = "munge"
  }, ]
  additional_disks = concat(var.additional_disks, var.munge_disk != null ? local.munge_disk : [])
