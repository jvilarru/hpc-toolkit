...
| <a name="input_hybrid_conf"></a> [hybrid\_conf](#input\_hybrid\_conf) | The hybrid configuration | <pre>object({<br>    slurm_bin_dir           = optional(string) #done<br>    slurm_log_dir           = optional(string) #done<br>    slurm_control_host      = string<br>    slurm_control_host_port = optional(string) #done<br>    slurm_control_addr      = optional(string) #done<br>    output_dir              = optional(string) #done<br>    install_dir             = optional(string) #done<br>    slurm_uid               = optional(number)<br>    slurm_gid               = optional(number)<br>    munge_secret            = string<br>  })</pre> | `null` | no |
