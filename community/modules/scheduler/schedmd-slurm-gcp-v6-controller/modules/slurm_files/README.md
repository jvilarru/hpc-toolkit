...
| [google_storage_bucket_object.dbd_startup_scripts](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/storage_bucket_object) | resource |
...
| <a name="input_dbd_startup_scripts"></a> [dbd\_startup\_scripts](#input\_dbd\_startup\_scripts) | List of scripts to be ran on dbd VM startup. | <pre>list(object({<br>    filename = string<br>    content  = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_slurm_dbd_host"></a> [slurm\_dbd\_host](#input\_slurm\_dbd\_host) | The short, or long, hostname of the machine where Slurm DB daemon is<br>executed (i.e. the name returned by the command "hostname -s").<br><br>This value is passed to slurm.conf such that:<br>AccountingStorageHost={var.slurm\_dbd\_host}<br><br>A null value specifies to use the controller as the Slurm DB daemon. | `string` | `null` | no |
