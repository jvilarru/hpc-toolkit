...
module "slurm_dbd_instance" {
  source = "github.com/GoogleCloudPlatform/slurm-gcp.git//terraform/slurm_cluster/modules/_slurm_instance?ref=6.6.2"

  access_config       = var.enable_dbd_public_ips ? [local.access_config] : []
  add_hostname_suffix = false
  hostname            = "${local.slurm_cluster_name}-dbd"
  instance_template   = module.slurm_dbd_template.self_link
...
locals {
  munge_secret = var.munge_secret != null ? var.munge_secret : "${local.slurm_cluster_name}-slurm-secret-munge"
  jwt_secret   = var.jwt_secret != null ? var.jwt_secret : "${local.slurm_cluster_name}-slurm-secret-jwt"
}
...
# SECRETS: MUNGE
resource "random_bytes" "munge_secret" {
  length = 1024
}

resource "google_secret_manager_secret" "munge" {
  count     = var.munge_secret == null ? 1 : 0
  secret_id = local.munge_secret
...
resource "google_secret_manager_secret_version" "munge_version" {
  count                 = var.munge_secret == null ? 1 : 0
  secret                = google_secret_manager_secret.munge[0].id
  secret_data           = random_bytes.munge_secret.base64
  is_secret_data_base64 = true
}

resource "google_secret_manager_secret_iam_member" "munge_secret_accessor" {
  count     = var.jwt_secret == null ? 1 : 0
  secret_id = google_secret_manager_secret.munge[0].id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${local.service_account.email}"
}
...
resource "google_secret_manager_secret_version" "jwt_version" {
  count                 = var.jwt_secret == null ? 1 : 0
  secret                = google_secret_manager_secret.jwt[0].id
  secret_data           = random_bytes.jwt_secret.base64
  is_secret_data_base64 = true
}

resource "google_secret_manager_secret_iam_member" "jwt_secret_accessor" {
  count     = var.jwt_secret == null ? 1 : 0
  secret_id = google_secret_manager_secret.jwt[0].id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${local.service_account.email}"
}
